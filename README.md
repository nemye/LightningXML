...
|---|---|---|---|---|---|---|---|
| Flat (2K items, 4 fields + attr) | **3.82 GB/s** | 2.61 GB/s | 1.46 GB/s | 199 MB/s | 1.58 GB/s | 178 MB/s | 189 MB/s |
| Deep (2K chains, 5 levels) | **1.72 GB/s** | 1.57 GB/s | 977 MB/s | 792 MB/s | 1.05 GB/s | 101 MB/s | 124 MB/s |
| Attributes (2K items, 10 attrs) | **1.91 GB/s** | 921 MB/s | 417 MB/s | 719 MB/s | 886 MB/s | 34 MB/s | 101 MB/s |
| Small (1 element) | **1.95 GB/s** | 1.55 GB/s | 823 MB/s | 886 MB/s | 1.13 GB/s | 68 MB/s | 76 MB/s |
| Large (10K users) | **3.31 GB/s** | 2.36 GB/s | 311 MB/s | 194 MB/s | 340 MB/s | 60 MB/s | 172 MB/s |
| Org (nested, ~400 members) | **1.91 GB/s** | 1.56 GB/s | 785 MB/s | 821 MB/s | 966 MB/s | 135 MB/s | 158 MB/s |
| Tree (depth 14, binary) | **805 MB/s** | 785 MB/s | 300 MB/s | 113 MB/s | 112 MB/s | 115 MB/s | 118 MB/s |
| Comment-heavy (skipped bytes) | **13.22 GB/s** | 7.99 GB/s | 2.87 GB/s | 2.58 GB/s | 3.25 GB/s | 506 MB/s | 590 MB/s |
| Catalog (12 books, owning strings) | **2.71 GB/s** | 1.82 GB/s | 1.33 GB/s | 1.15 GB/s | 1.54 GB/s | 190 MB/s | 215 MB/s |
...
LightningXML leads on every workload (1.8–11× over pugixml). The depth-14 Tree row is allocation-bound and high-variance for the DOM parsers (pugixml/RapidXML), so treat those cells as approximate. The LightningXML columns were re-measured after the constant-length tag matching and streamed attribute capture optimizations; the comparison columns are from the prior run on the same machine and toolchain (the comparison libraries were unchanged). All comparison benchmarks are opt-in CMake options and live in [test/bench_LightningXML.cc](test/bench_LightningXML.cc); see the per-section comments there for the exact fairness choices.
